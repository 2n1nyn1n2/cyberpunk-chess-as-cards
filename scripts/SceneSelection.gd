...
const POSSESSION_X_OFFSET: int = 128
const POSSESSION_Y_OFFSET: int = 128
const POSSESSION_X_MAX: int = 2
...
func _ready():
	...
	if GlobalState.do_test_moves:
		...
		#ChessLogic.make_move_notation("a4", "b5")
		#ChessLogic.make_move_notation("c8", "b7")
		#ChessLogic.force_ai_move()
		GlobalState.move_from = "a4"
		GlobalState.move_to = "b5"
...
func load_possessions_from_json(path: String):
	deck_possessions.clear()
	var data = ActiveRecords.get_active_dictionary_records(path, "possessions")
	if typeof(data) == TYPE_ARRAY:
		for c in data:
			var possession_name = c.get("name", "Unknown")
			var textureDir = "res://assets/possessions/" + possession_name
			var possession_image_texture = ActiveRecords.get_random_png_resource_from_dir(
				textureDir
			)
			...
			#print("possession.possession_name:", possession.possession_name)
...
func remove_all_possessions_children():
	var children = possessions.get_children()
	for child in children:
		if is_instance_valid(child):
			possessions.remove_child(child)


func draw_possessions():
	remove_all_possessions_children()
...
func _on_ai_finished(_best_move, _recommended_move):
	...
	#ChessLogic.make_move_notation("b7", "b5")
	#print("_on_ai_finished: ", " ChessLogic.captured_white", ChessLogic.captured_white)
	#print("_on_ai_finished: ", " ChessLogic.captured_black", ChessLogic.captured_black)
	possessions_data.clear()
	for ix in range(ChessLogic.captured_white.size()):
		var piece = ChessLogic.captured_white[ix]
		increment_key(possessions_data, piece.color + "_" + piece.type)
	for ix in range(ChessLogic.captured_black.size()):
		var piece = ChessLogic.captured_black[ix]
		increment_key(possessions_data, piece.color + "_" + piece.type)

	#var data = ActiveRecords.get_active_dictionary_records("res://data/possessions.json", "possessions")
	#if typeof(data) == TYPE_ARRAY:
	#for c in data:
	#var possession_name = c.get("name", "Unknown")
	#increment_key(possessions_data, possession_name)

	print("_on_ai_finished: ", " possessions_data", possessions_data)
...
func increment_key(dict: Dictionary, key: String):
	if dict.has(key):
		dict[key] += 1
	else:
		dict[key] = 1
