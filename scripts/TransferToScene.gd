...
func transfer_data(
	...
	var scenes_data = ActiveRecords.get_active_dictionary_records(
		"res://data/scenes.json", "scenes"
	)
	var scene_tile_to_transfer = "Unknown"
	if typeof(scenes_data) == TYPE_ARRAY and scenes_data.size() > 0:
		for scene_data in scenes_data:
			var scene_name = scene_data.get("name", "Unknown")
			if scene_name_to_transfer == scene_name:
				var tile = scene_data.get("tile", "Unknown")
				scene_tile_to_transfer = tile

	var error = get_tree().change_scene_to_file(scene_path)
	if error != OK:
		if OS.is_debug_build():
			push_error("Error changing scene: %s Error Code: %d" % [scene_path, error])
	...
	if new_scene_root == null:
		push_error("Null current_scene error when changing scene to: %s" % [scene_path])
	...
	if challenge_name_to_transfer != "":
		...
		if placeholder_challenge and is_instance_valid(placeholder_challenge):
			...
			placeholder_challenge.challenge_tile = scene_tile_to_transfer
	...
	new_scene_root.commit_to_transfer()
