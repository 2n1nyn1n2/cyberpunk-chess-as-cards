	...
	for scene in scenes:
		...
		if chess_moves.has(scene_name):
			...
			if !chess_piece.is_empty():
				...
				challenge.challenge_name = scene_name
				challenge.challenge_tile = tile
				challenge.challenge_description = "empty_" + tile

				var textureDir = "res://assets/scenes/" + scene_name
				challenge.challenge_image_texture = ActiveRecords.get_random_png_resource_from_dir(
					textureDir
				)
				challenge.update_ui()
			...
			if !chess_piece.is_empty():
				...
				for challenge_data in challenge_array:
					...
					if chess_piece_challenge_name == challenge_name:
						...
						challenge.challenge_description = challenge_data.get(
							"description", "Unknown"
						)
						var textureDir = "res://assets/challenges/" + challenge_name
						challenge.challenge_image_texture = (
...
func _on_back_button_pressed():
	TransferToScene.transfer_data(
		...
		background.background_name,
		selected_challenge.challenge_name if is_instance_valid(selected_challenge) else "",
		possessions_data,
