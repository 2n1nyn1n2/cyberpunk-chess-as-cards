...
func initialize_board() -> Dictionary:
	...
	for i in range(8):
		var w_back = Vector2(0, i)
		var w_pawn = Vector2(1, i)
		new_board[w_back] = {
			"type": piece_order[i], "color": "white", "moved": false, "origin": w_back
		}
		new_board[w_pawn] = {"type": "pawn", "color": "white", "moved": false, "origin": w_pawn}

		var b_pawn = Vector2(6, i)
		var b_back = Vector2(7, i)
		new_board[b_pawn] = {"type": "pawn", "color": "black", "moved": false, "origin": b_pawn}
		new_board[b_back] = {
			"type": piece_order[i], "color": "black", "moved": false, "origin": b_back
		}
	return new_board
...
func make_move(from: Vector2, to: Vector2) -> bool:
	...
	if to in valid_moves:
		en_passant_target = _execute_move_on_board(board, from, to, en_passant_target, false)
		_toggle_turn()
...
func start_ai_thread() -> void:
	...
	var ep_copy = en_passant_target
	WorkerThreadPool.add_task(func(): make_ai_move(board_copy, ep_copy))


func make_ai_move(sim_board: Dictionary, start_ep: Vector2) -> void:
	var best_move = null
	...
	for m in moves:
		...
		var next_ep = _execute_move_on_board(temp_board, m.from, m.to, start_ep, true)
		var score = -_minimax(temp_board, depth - 1, -1000000, 1000000, opponent_color, next_ep)

		if score > best_score:
			best_score = score
			best_move = m

	# Calculate recommended white move (Best response for white)
	if best_move:
		var response_sim = sim_board.duplicate(true)
		var resp_ep = _execute_move_on_board(
			response_sim, best_move.from, best_move.to, start_ep, true
		)
		var white_moves = _get_all_moves_with_context(response_sim, "white", resp_ep)
		var best_white_score = -1000000
		for wm in white_moves:
			var temp_white = response_sim.duplicate(true)
			_execute_move_on_board(temp_white, wm.from, wm.to, resp_ep, true)
...
func _minimax(
	...
	for m in moves:
		...
		var next_ep = _execute_move_on_board(temp, m.from, m.to, ep, true)
		var eval = -_minimax(temp, depth - 1, -beta, -alpha, next_side, next_ep)
...
func _evaluate_board(b: Dictionary, perspective_color: String) -> int:
	...
	for pos in b:
		...
		total_eval += val if p.color == perspective_color else -val
	return total_eval
...
func _execute_move_on_board(
	b: Dictionary, from: Vector2, to: Vector2, current_ep: Vector2, is_simulated: bool
) -> Vector2:
	if not b.has(from):
		return Vector2(-1, -1)
	var piece = b[from]
	...
	if b.has(to):
		if not is_simulated:
			_record_capture(b[to], to)
	elif piece.type == "pawn" and to == current_ep:
		var ep_pawn_pos = Vector2(from.x, to.y)
		if b.has(ep_pawn_pos):
			if not is_simulated:
				_record_capture(b[ep_pawn_pos], ep_pawn_pos)
			b.erase(ep_pawn_pos)
...
func _record_capture(captured_piece: Dictionary, death_coord: Vector2) -> void:
	var piece_data = {
		"type": captured_piece.type,
		"color": captured_piece.color,
		"origin": coord_to_notation(captured_piece.origin),
		"captured_at": coord_to_notation(death_coord)
	}
	if captured_piece.color == "white":
		captured_white.append(piece_data)
	else:
		captured_black.append(piece_data)


func _finalize_ai_move(m, recommended_move):
	_set_ai_thinking(false)
	if m:
		en_passant_target = _execute_move_on_board(board, m.from, m.to, en_passant_target, false)
		_toggle_turn()
...
func _will_be_in_check(
	from: Vector2, to: Vector2, color: String, b: Dictionary, ep: Vector2
) -> bool:
	var sim = b.duplicate(true)
	_execute_move_on_board(sim, from, to, ep, true)
