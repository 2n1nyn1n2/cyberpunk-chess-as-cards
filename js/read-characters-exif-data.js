...
async function readExifDataForFile(filePath) {
  try {
    ...
    if (inputTags.UserComment) {
      ...
      const character_name = filePath.split("_characters_")[1].split("_0__")[0];
      ...
      const dir = path.join("assets", "characters", character_name);
      if (!fs.existsSync(dir)) {
        fs.mkdirSync(dir);
      }

      const newFilePath = path.join(dir, `${character_name}_${seed}.png`);
