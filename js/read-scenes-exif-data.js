...
async function readExifDataForFile(filePath) {
  try {
    ...
    if (inputTags.UserComment) {
      ...
      const scene_name = filePath.split("_scenes_")[1].split("_0__")[0];
      ...
      const dir = path.join("assets", "scenes", scene_name);
      if (!fs.existsSync(dir)) {
        fs.mkdirSync(dir);
      }

      const newFilePath = path.join(dir, `${scene_name}_${seed}.png`);
