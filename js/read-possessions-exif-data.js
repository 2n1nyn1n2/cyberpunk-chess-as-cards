...
async function readExifDataForFile(filePath) {
  try {
    ...
    if (inputTags.UserComment) {
      ...
      const possession_name = filePath
        .split("_possessions_")[1]
        .split("_0__")[0];
      ...
      console.log(`\tPossession:`, possession_name);

      const dir = path.join("assets", "possessions", possession_name);
      if (!fs.existsSync(dir)) {
        fs.mkdirSync(dir);
      }

      const newFilePath = path.join(dir, `${possession_name}_${seed}.png`);
