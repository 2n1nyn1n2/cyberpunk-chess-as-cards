...
async function readExifDataForFile(filePath) {
  try {
    ...
    if (inputTags.UserComment) {
      ...
      const challenge_name = filePath.split("_challenges_")[1].split("_0__")[0];

      console.log(`✅ File: ${filePath}`);
      console.log(`\tSeed:`, seed);
      // console.log(`\tPrompt:`, prompt);
      console.log(`\tModel`, model);
      console.log(`\tChallenge:`, challenge_name);

      const dir = path.join("assets", "challenges", challenge_name);
      if (!fs.existsSync(dir)) {
        fs.mkdirSync(dir);
      }

      const newFilePath = path.join(dir, `${challenge_name}_${seed}.png`);
